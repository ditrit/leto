...
export default {
	...
	setup(_props, { emit }) {
		...
		function dragstarted($event) {
			// delay any action until the actual drag start
			EventBus.on(`modelization:realdragstarted`, currentModel => {
				if (currentModel.id !== this.parentNode.id) {
					EventBus.off(`modelization:realdragstarted`)
					return;
				}
		...
		function dragged(event) {
			EventBus.emit(`modelization:realdragstarted`, this.parentNode);
