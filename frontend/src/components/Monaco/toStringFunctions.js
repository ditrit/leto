export function toStringResource(resource, type, container) {
	...
	resourceToString += (resource.links) ? toStringLinks(resource.links.outputs, true, resource.objects.value) : '';
	resourceToString += (resource.links) ? toStringLinks(resource.links.inputs, false, resource.objects.value) : '';
...
function toStringLinks(links, output, objects) {
	let res = '';
    let variableNames = [];
	...
	links.forEach( link => {
      ...
      if(link.multiple && !variableNames.includes(link.variableName)) {
          ...
          ele += toStringArray(links, link.variableName, output);
      } else if(!link.multiple) {
          ele += `  ${link.variableName} = ${link.targetType}.${link.targetName}\n`;
      }
      ...
      if(link.multiple && !variableNames.includes(link.variableName)) {
          ...
          ele += `  ${link.variableName} = ${link.sourceType}.${link.sourceName}\n`;
      }
    }
		let found = false;
